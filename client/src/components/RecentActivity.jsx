...
const RecentActivity = ({ address, txFilter }) => {
  ...
  //  const copy = () => {
  //     navigator.clipboard.writeText(allTxs?.lockTxHash);
  //     setCopied(true);
  ...
  const filtered = allTxs.filter(tx => {
    ...
    if (txFilter === "receive") return tx.status === "DEPOSIT";
    if (txFilter === "claim") return tx.status === "BRIDGE";
    if (txFilter === "confirmed") return tx.status === "PAID";
    if (txFilter === "pending") return tx.status === "LOCKED";
    if (txFilter === "Claimed") return tx.status === "CLAIMED";
    return true;
  });
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {filtered?.map((tx, i) => (
            <tr
              key={tx.id}
              className={`border-b border-green-50 hover:bg-green-50/60 transition-colors ${i % 2 === 0 ? "bg-white" : "bg-green-50/20"
                }`}
            >
              ...
              <td className="px-4 py-3.5 font-bold text-green-900">
                {tx.lockedAmount}{" "}
                <span
                  className={`text-xs ${tx.tokenSymbol === "ETH"
                      ? "text-blue-600 bg-blue-50 py-1 px-0.5 rounded-md"
                      : "text-green-500"
                    }`}
                >
                  {tx.tokenSymbol}
              ...
              <td className="px-4 py-3.5">
                <a
                  href={`https://sepolia.explorer.zksync.io/tx/${tx.lockTxHash}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="mono text-xs text-zinc-600"
                >
                  {trunc(tx.lockTxHash)}
                </a>
              </td>
              ...
              <td className="px-4 py-3.5">
                <a
                  href={`https://sepolia.explorer.zksync.io/tx/${tx.payoutTxHash}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="mono text-xs text-zinc-600"
                >
                  {trunc(tx.payoutTxHash)}
                </a>
              </td>
